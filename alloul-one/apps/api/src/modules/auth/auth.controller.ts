...
export class AuthController {
  ...
  passwordlessVerify(@Body() body: { email: string; code: string }) {
    ...
    const accessToken = this.jwt.sign({ sub: body.email, role: 'owner', permissions: ['org.read', 'org.write', 'rbac.read', 'knowledge.read', 'knowledge.write', 'knowledge.approve', 'search.query', 'search.index', 'audit.read', 'audit.write', 'audit.verify', 'project.read', 'project.write', 'task.read', 'task.write', 'workflow.read', 'workflow.write', 'workflow.run', 'handover.read', 'handover.write'] });
